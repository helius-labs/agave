...
fn bench_wakeup_latency(c: &mut Criterion) {
    ...
    // --- park/unpark (our implementation) ---
    group.bench_function("park_unpark", |b| {
        ...
        notify.notify(); // unpark so it exits
        barrier.wait();
...
fn bench_notify_contention(c: &mut Criterion) {
    ...
    for num_contenders in [0, 4, 8] {
        ...
        group.bench_with_input(
            BenchmarkId::new("park_unpark", num_contenders),
            &num_contenders,
            |b, &n| {
                let notify = Arc::new(VoteNotify::new());
                notify.register_thread(); // register current thread (won't actually park)
